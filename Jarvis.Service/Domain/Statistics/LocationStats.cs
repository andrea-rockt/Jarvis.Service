    ...
    public class LocationStats :Entity<Guid>
    {
        ...
        public virtual TimeSpan GetTotalTimeSpent() { 
            ...
            if (!enterEntries.Any() || !exitEntries.Any() )
            {
                ...
                var resultTimespan =
                    enterEntries.Zip(exitEntries, (enter, exit) =>
                                                      {
                                                          if (enter.Date.CompareTo(exit.Date)>0)throw new InvalidOperationException("Location Statistics data is inconsistent");
